...
TEST_F(TupleTest, StaticExplicit) {
...
//         = static_cast<T& (*)(Tuple<int, double, char>&)> ( &Get<T, int, double, char>)) {
//     return 0;
// }
//
...
TEST_F(TupleTest, StaticConstructionTests) {
...
TEST_F(TupleTest, DefaultConstructible) {
    {
        Tuple<int, int> t1;
        ASSERT_EQ(Get<0>(t1), 0);
        ASSERT_EQ(Get<1>(t1), 0);
    }
    ...
    ASSERT_EQ(Accountant::destructed, 2);
}

TEST_F(TupleTest, ConstructionBasic) {
    {
        Accountant a;
        Tuple<Accountant, int> t(a, 5);
        ASSERT_EQ(Accountant::copy_constructed, 1);

        Tuple t2 = t;
        ASSERT_EQ(Accountant::copy_constructed, 2);

        Get<1>(t) = 7;
        ASSERT_EQ(Get<1>(t), 7);
        ASSERT_EQ(Get<1>(t2), 5);

        t = t2;
        ASSERT_EQ(Accountant::copy_assigned, 1);
        ASSERT_EQ(Accountant::copy_constructed, 2);
        ASSERT_EQ(Get<1>(t), 5);
        ASSERT_EQ(Get<1>(t2), 5);

        t2 = std::move(t);
        ASSERT_EQ(Accountant::move_assigned, 1);
        ASSERT_EQ(Accountant::copy_assigned, 1);
        ASSERT_EQ(Accountant::move_constructed, 0);
        ASSERT_EQ(Accountant::copy_constructed, 2);

        const auto& t3 = t;
        t2 = std::move(t3);
        ASSERT_EQ(Accountant::move_assigned, 1);
        ASSERT_EQ(Accountant::copy_assigned, 2);
    }

    ASSERT_EQ(Accountant::destructed, 3);
}

TEST_F(TupleTest, ConstructionRef) {
    int x = 5;
    double d = 3.14;
...
TEST_F(TupleTest, ConstructionLvalueRef) {
    {
        Accountant a;
        int x = 5;
        Tuple<Accountant&, const int&> t(a, x);
        ASSERT_EQ(Accountant::copy_constructed, 0);
        ASSERT_EQ(Get<1>(t), 5);
        ...
        t3 = std::move(t2);
        ASSERT_EQ(Accountant::move_assigned, 1);
        ASSERT_EQ(Accountant::copy_assigned, 0);
        ...
        ASSERT_EQ(Accountant::move_assigned, 1);
        ASSERT_EQ(Accountant::copy_assigned, 1);
        ASSERT_EQ(Get<1>(t4), 7);
    }

    ASSERT_EQ(Accountant::destructed, 7);
}

TEST_F(TupleTest, ConstructionRvalueRef) {
    {
        Accountant a;
        int x = 5;
        ...
        ASSERT_EQ(Accountant::copy_constructed, 1);
        ASSERT_EQ(Accountant::move_constructed, 3);

        x = 15;
        t4 = t3;
        ASSERT_EQ(Accountant::copy_assigned, 1);
        ASSERT_EQ(Accountant::move_assigned, 0);
        ASSERT_EQ(Get<1>(t4), 15);

        t5 = std::move(t3);
        ASSERT_EQ(Accountant::copy_assigned, 1);
        ASSERT_EQ(Accountant::move_assigned, 1);
        ASSERT_EQ(Get<1>(t5), 15);
        ...
        t3 = std::move(t5);
        ASSERT_EQ(Accountant::copy_assigned, 2);
        ASSERT_EQ(Accountant::move_assigned, 2);
        ASSERT_EQ(x, 33);
    }

    ASSERT_EQ(Accountant::destructed, 5);
}

TEST_F(TupleTest, ConstructionMoveOnly) {
    int x = 5;
...
TEST_F(TupleTest, RefPreservation) {
    int x = 5;
    ...
    x = 3;
    ASSERT_EQ(Get<1>(tr), 3);
    ASSERT_EQ(Get<1>(t2), 5);
}

TEST_F(TupleTest, ConstructionFromPair) {
...
TEST_F(TupleTest, MatchingRefs) {
    ...
    ++Get<int&>(t);
    ++Get<int&&>(t);
    ASSERT_EQ(Get<const int&>(t), 3);
    ASSERT_EQ(Get<int>(t), 2);
    ASSERT_EQ(Get<3>(t), 3);
    ASSERT_EQ(x, 3);
}

TEST_F(TupleTest, GetWithModifiers) {
    int x = 3;
    int y = 4;
...
TEST_F(TupleTest, GetByType) {
    Tuple<int> first(12);
    auto second = first;
    ASSERT_EQ(Get<0>(first), Get<0>(second));

    second = Tuple<int>(14);
    ASSERT_EQ(Get<0>(second), 14);
    ...
    ASSERT_EQ(Get<int>(second), 3);

    first = 1;
    ASSERT_EQ(Get<int>(first), 1);

    // For none or multiple choice there should be a compilation error via SFINAE or
    // constraints
...
TEST_F(TupleTest, MakeTuple) {
    ...
    Tuple tuple =
        MakeTuple(5, test, std::array<int, 2>{5, 5}, NeitherDefaultNorCopyConstructible(1));

    Get<1>(tuple)[1] = 'i';
    ASSERT_EQ(Get<1>(tuple), "tist");
    ASSERT_EQ(test, "test");

    Get<2>(tuple)[1] = 2;
    ASSERT_EQ(Get<2>(tuple)[0], 5);
    ASSERT_EQ(Get<2>(tuple)[1], 2);
}

TEST_F(TupleTest, RefWrapper) {
    {
        int value = 0;
        Tuple tuple(5, std::reference_wrapper{value}, value);
        static_assert(
            std::is_same_v<decltype(tuple), Tuple<int, std::reference_wrapper<int>, int>>);
    ...
    {
        int value = 0;
        Tuple tuple = MakeTuple(5, std::reference_wrapper{value}, value);
        static_assert(std::is_same_v<decltype(tuple), Tuple<int, int&, int>>);
...
TEST_F(TupleTest, Tie) {
    int value = 5;
    std::string test("test");

    Tie(value, test) = Tuple(3, std::string("amogus"));
    ASSERT_EQ(value, 3);
    ASSERT_EQ(test, "amogus");

    static_assert(!std::is_assignable_v<decltype(Tie<const int>(std::declval<int>())), int>);
}

TEST_F(TupleTest, ForwardAsTuple) {
    int value = 1;
...
TEST_F(TupleTest, TupleCat) {
    ...
    ASSERT_EQ(Accountant::move_constructed, 3);
    ASSERT_EQ(Accountant::copy_constructed, 5);
}
